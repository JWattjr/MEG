...
export function MatchesClient() {
  ...
  useEffect(() => { void readGameRounds().then(setRounds).catch((caught) => setError(caught instanceof Error ? caught.message : "Unable to read registered StudioNet rounds.")); }, []);
  ...
  return <div className="round-list">
    ...
    {genLayerGameConfig.enabled && !error && rounds.length === 0 && <div className="empty-state"><strong>No registered MEG rounds yet.</strong><span>Fresh StudioNet rounds appear here after a deployer registers the market and evidence policy.</span></div>}
    {!genLayerGameConfig.enabled && <div className="notice-strip"><ShieldCheck size={15} /><span>No MEG StudioNet contract is configured in this local build. Add the fresh addresses from `.env.local` to read real rounds.</span></div>}
  </div>;
}
