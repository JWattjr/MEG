...
The game contract is deliberately market-pack agnostic. The TypeScript market
schema assigns the nine Valorant cell definitions and options; this contract
only accepts one option from each fixed cell, keeps every ledger isolated, and
settles the resolver's column bitmaps deterministically.
"""
...
def _validate_grid(packed_grid: u256) -> None:
    ...
    for cell in range(CELLS):
        ...
        if moment_id < first or moment_id >= first + OPTIONS_PER_CELL:
            raise gl.vm.UserError("Market option is not valid for its grid cell")
