...
- Reusable frontend foundation: wallet controls, Studio Next client helpers, on-chain round/entry reads, Guided Play patterns, route shell, and responsive game composition.
...
4. Rebuild the web UI as an original MEG broadcast-operations desk: grid-first, cobalt/acid-lime/cream, mobile-first, keyboard accessible, and honest about synthetic data versus Studio Next state.
...
6. Add documentation, verified fixture data, Studio Next-only environment defaults, bot disclosure/runbook notes, and direct/unit/e2e coverage.
7. Run linting, type checking, builds, direct tests, and one bounded desktop/mobile browser verification pass. Deploy only after all gates pass; never fund bots automatically.
...
- Verified fixtures identify their sources and pending fields; synthetic fallbacks are visibly labeled and never presented as live fixtures.
- Studio Next is the only supported public network; localnet is test-only.
